import AppScreenshot from "@/assets/img/screenshot.png";
import Vector from "@/assets/img/Vector.png";
import Vector2 from "@/assets/img/Vector2.png";
import freelancers from "@/assets/img/freelancers.png";
import upwork from "@/assets/img/upwork.png";
import proposals from "@/assets/img/proposals.png";
import type { HeroConfig } from "@/types/hero";

export const HERO_CONFIG: HeroConfig = {
  subtitle: "Designed to Turn Profile Views Into Interviews",
  title: "Win More Jobs on Upwork",
  titleHighlight: "Automatically",
  description: "Provolo uses AI + proven copywriting strategies to optimize your profile and proposals, so you can land clients faster.",
  cta: {
    text: "Optimize My Profile",
    href: "/signup",
  },
  screenshot: {
    alt: "Provolo app dashboard",
    src: AppScreenshot,
  },
  floatingImages: {
    proposals: {
      alt: "AI proposal optimization",
      src: proposals,
      className: "absolute lg:-top-24 -bottom-10 lg:-right-24 right-0 lg:w-32 w-[100px]",
    },
    freelancers: {
      alt: "Freelancers using AI",
      src: freelancers,
      className: "absolute lg:-left-48 left-5 -top-14 lg:w-32 w-[100px]",
    },
    upwork: {
      alt: "Upwork profile optimization",
      src: upwork,
      className: "absolute w-[100px] lg:w-32 -bottom-20 left-0 lg:left-auto lg:-right-96 lg:top-32",
    },
  },
  backgroundVectors: {
    vector1: {
      alt: "",
      src: Vector,
      className: "absolute top-0 left-0 lg:w-1/5 xl:w-1/6 w-1/2 opacity-80",
    },
    vector2: {
      alt: "",
      src: Vector2,
      className: "absolute bottom-0 right-0 w-1/3 opacity-80",
    },
  },
};

export const HERO_ANIMATIONS = {
  container: {
    hidden: { opacity: 0 },
    visible: {
      opacity: 1,
      transition: {
        delayChildren: 0.1,
        staggerChildren: 0.08,
      },
    },
  },
  item: {
    hidden: { opacity: 0, y: 15 },
    visible: {
      opacity: 1,
      y: 0,
      transition: {
        duration: 0.5,
        ease: "easeOut",
      },
    },
  },
  floating: {
    animate: {
      y: [-5, 5, -5] as number[],
      rotate: [-1, 1, -1] as number[],
      transition: {
        duration: 5,
        ease: "easeInOut",
        repeat: Infinity,
        repeatType: "reverse" as const,
      },
    },
  },
  vector: {
    hidden: { opacity: 0, scale: 0.95 },
    visible: {
      opacity: 1,
      scale: 1,
      transition: {
        duration: 0.6,
        ease: "easeOut",
        delay: 0.2,
      },
    },
  },
  screenshot: {
    hidden: { opacity: 0, y: 40, scale: 0.98 },
    visible: {
      opacity: 1,
      y: 0,
      scale: 1,
      transition: {
        duration: 0.7,
        ease: "easeOut",
        delay: 0.5,
      },
    },
  },
  ctaHover: {
    scale: 1.05,
    transition: { duration: 0.15 },
  },
  ctaTap: {
    scale: 0.98,
  },
  screenshotHover: {
    y: -8,
    transition: { duration: 0.3 },
  },
  floatingHover: {
    scale: 1.1,
    transition: { duration: 0.2 },
  },
} as const;

export const HERO_STYLES = {
  container: "min-h-screen lg:h-screen h-[800px] overflow-hidden relative bg-light flex flex-col items-center justify-center pt-20 lg:pt-24",
  content: "relative isolate px-6 lg:px-8 z-10 lg:pb-10 pb-44",
  inner: "mx-auto max-w-[75rem] flex flex-col lg:gap-[60px] gap-[100px]",
  textContainer: "text-center lg:w-[629px] mx-auto flex flex-col lg:gap-[20px] gap-[20px] relative items-center",
  subtitle: "lg:text-sm text-[10px] uppercase tracking-wider",
  title: "tracking-tight leading-tight text-balance lg:text-xl md:text-7xl text-4xl font-medium",
  titleHighlight: "font-headingmd text-primary",
  description: "text-[#6B7280] lg:text-base text-[15px] w-[80%] mx-auto",
  screenshotContainer: "absolute z-10 mx-auto w-[1300px] max-w-[1400px] -bottom-[500px] md:-bottom-[500px] lg:-bottom-[550px] lg:left-0 lg:right-0 left-8 rounded-[32px] bg-white p-4 shadow-lg",
  screenshot: "w-full rounded-[22px] border object-cover",
} as const;
