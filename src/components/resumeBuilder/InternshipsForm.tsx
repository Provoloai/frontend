      ...
      >
        ...
        >
          <div className="flex items-center gap-3 pr-8">
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${isExpanded ? "bg-orange-100" : "bg-gray-200"
              }`}>
              <Briefcase className={`w-4 h-4 ${isExpanded ? "text-orange-600" : "text-gray-500"}`} />
            </div>
...
export const InternshipsForm: React.FC<InternshipsFormProps> = ({ control, watch, setValue }) => {
  ...
  const handleReorder = (newOrder: any[]) => {
    setValue('internships', newOrder, { shouldDirty: true, shouldValidate: true });
  };
    ...
    >
      ...
      {internships.length === 0 && (
        ...
        >
          ...
          <p className="text-xs text-gray-500">Click "Add Internship" to get started</p>
        </motion.div>
      )}
    </motion.div>
  );
