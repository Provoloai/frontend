...
import { Trash2, GraduationCap, Calendar, MapPin, School, ChevronDown, ChevronUp, GripVertical, Lightbulb } from "lucide-react";
        ...
        >
          <div className="flex items-center gap-3 pr-8">
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${isExpanded ? "bg-blue-100" : "bg-gray-200"
              ...
              <GraduationCap className={`w-4 h-4 ${isExpanded ? "text-blue-600" : "text-gray-500"}`} />
            </div>
...
const EducationForm: React.FC<EducationFormProps> = ({ control, watch, setValue }) => {
  ...
  const handleReorder = (newOrder: any[]) => {
    setValue('education', newOrder, { shouldDirty: true, shouldValidate: true });
  };
